...
from lava_dispatcher.pipeline.actions.test.monitor import TestMonitorRetry
...
class MultinodeTest(LavaTest):
    ...
    def __init__(self, parent, parameters):
        super(MultinodeTest, self).__init__(parent)
        if 'monitors' in parameters:
            self.action = MultinodeTestMonitorAction()
            self.deployment_data = False
            self.overlay = False
            self.shell = False
        else:
            self.action = MultinodeTestShellAction()
            self.deployment_data = True
            self.overlay = True
            self.shell = True
...
class MultinodeTestMonitorAction(TestMonitorRetry):

    def __init__(self):
        super(MultinodeTestMonitorAction, self).__init__()
        self.name = "multinode-test-monitor"
        self.description = "Executing lava-test-monitor"
        self.summary = "Multinode Lava Test Monitor"
        self.multinode_dict = {
            'multinode': r'<LAVA_MULTI_NODE> <LAVA_(\S+) ([^>]+)>',
        }

    def validate(self):
        super(MultinodeTestMonitorAction, self).validate()
    ...
    def populate(self, parameters):
        ...
        """
        super(MultinodeTestMonitorAction, self).populate(parameters)
    ...
    def check_patterns(self, event, test_connection, check_char):
        ...
        """
        ret = super(MultinodeTestMonitorAction, self).check_patterns(event, test_connection, check_char)
        if event == 'multinode':
    ...
    class SignalDirector(TestShellAction.SignalDirector):
        ...
        def __init__(self, protocol):
            super(MultinodeTestMonitorAction.SignalDirector, self).__init__(protocol)
            self.base_message = {}
...
class MultinodeTestShellAction(TestShellAction):

    def __init__(self):
        super(MultinodeTestShellAction, self).__init__()
        self.name = "multinode-test-shell"
    ...
    def validate(self):
        super(MultinodeTestShellAction, self).validate()
        # MultinodeProtocol is required, others can be optional
        if MultinodeProtocol.name not in [protocol.name for protocol in self.job.protocols]:
            self.errors = "Invalid job - missing protocol"
        if MultinodeProtocol.name not in [protocol.name for protocol in self.protocols]:
    ...
    def _reset_patterns(self):
        super(MultinodeTestShellAction, self)._reset_patterns()
        self.patterns.update(self.multinode_dict)
    ...
    def populate(self, parameters):
        ...
        Select the appropriate protocol supported by this action from the list available from the job
        """
        super(MultinodeTestShellAction, self).populate(parameters)
        self.protocols = [protocol for protocol in self.job.protocols if protocol.name == MultinodeProtocol.name]
        self.signal_director = self.SignalDirector(self.protocols[0])
    ...
    def check_patterns(self, event, test_connection, check_char):
        ...
        Calls the parent check_patterns first, then checks for subclass pattern.
        """
        ret = super(MultinodeTestShellAction, self).check_patterns(event, test_connection, check_char)
        if event == 'multinode':
            name, params = test_connection.match.groups()
            self.logger.debug("Received Multi_Node API <LAVA_%s>" % name)
            params = params.split()
    ...
    class SignalDirector(TestShellAction.SignalDirector):
        ...
        def __init__(self, protocol):
            super(MultinodeTestShellAction.SignalDirector, self).__init__(protocol)
            self.base_message = {}
